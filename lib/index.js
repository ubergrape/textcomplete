...
Textcomplete.prototype.match = function () {
  var el = this.el
  var text = (el.nodeName == "textarea") ? el.value : el.innerHTML
  if (!text) return // nothing to match
  var index = (el.nodeName == "textarea") ? el.selectionEnd : document.getSelection().anchorOffset // cursor index
  var start = (el.nodeName == "textarea") ? el.selectionStart : document.getSelection().extentOffset // cursor start
...
Textcomplete.prototype.select = function (option) {
  ...
  var index = (el.nodeName == "textarea") ? el.selectionEnd : document.getSelection().anchorOffset
  var text = (el.nodeName == "textarea") ? el.value : el.innerHTML
  ...
  if (el.nodeName == "textarea") {
    ...
    el.innerHTML = start + text.slice(index)
    var new_el = query(':not(.checked)', el)
    Classes(new_el).add('checked')
    var caret = new Caret(el)
    caret.moveAfter(new_el)
  }
